...
{
    // https://codereview.stackexchange.com/questions/225530/c-tcp-server-client-class
    public class TcpConnection : IDisposable
    {
        ...
        private readonly Queue ReceivedByteQueue = new Queue();
            ...
            try
            {
                lock (syncLock)
                {
                    try
                    {
                        ...
                        if (ReceivedByteQueue.Count > 0)
                        {
                            ReceivedByteQueue.Clear();
                        }
                    }
                    catch { }
                }
        ...
        private void CopyReceived(byte receivedData)
        {
            try
            {
                lock (ReceivedByteQueue.SyncRoot)
                {
                    try
                    {
                        ReceivedByteQueue.Enqueue(receivedData);
                    }
                    catch { }
                }
            }
